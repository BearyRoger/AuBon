(function () {
    ...
    function slideTo(index) {
        ...
        $dots.forEach(($elt, key) => $elt.classList = `dot1 ${key === currentSlide? 'active': 'inactive'}`);
    }
    ...
    for (let i = 1; i <= $slides.length; i++) {
        ...
        let $dot = `<span data-slidId="${i}" class="dot1 ${dotClass}"></span>`;
        document.querySelector('.carousel1-dots').innerHTML += $dot;
    }
