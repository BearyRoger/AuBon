(function () {
    ...
    function slideTo(index) {
        ...
        $dots.forEach(($elt, key) => $elt.classList = `dot2 ${key === currentSlide? 'active': 'inactive'}`);
    }
    ...
    for (let i = 1; i <= $slides.length; i++) {
        ...
        let $dot = `<span data-slidId="${i}" class="dot2 ${dotClass}"></span>`;
        document.querySelector('.carousel2-dots').innerHTML += $dot;
    }
